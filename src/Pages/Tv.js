...
export default function Tv({ data }) {
	const [dataTv, setDataTv] = useState(data.filter((data) => data.category === "TV Series"));

	const dataHandler = (e) => {
		if (e.target.value.length >= 1) {
			setDataTv(data.filter((data) => data.title.toLowerCase().includes(e.target.value.toLowerCase())));
		} else {
			setDataTv(data.filter((data) => data.category === "Movie"));
		}
	};

	const tvCardsMapping = dataTv.map((data, idx) => (
	...
	return (
		<div className="home">
			...
			<div className="home__section">
				<p className="home__title">TV Series</p>
				{dataTv.length >= 1 ? (
